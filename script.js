// ==================== WALKIE-TALKIE SES EFEKTİ ====================
const audioCtx = new (window.AudioContext || window.webkitAudioContext)();

function playWalkieTone(type = 'start') {
    if (!audioCtx) return;
    ...
    if (type === 'start') {
        // BIP — yüksek frekanslı kısa "bip" (telsiz açıldı)
        osc.type = 'sine';
        ...
        // BOP — düşük frekanslı kısa "bop" (telsiz kapandı)
        osc.type = 'sine';
...
// ==================== DİL VERİSİ ====================
const LANG_DATA = {
    'auto':  { name: 'Otomatik Algıla', flag: '🔍', label: 'AUTO', api: 'auto', voice: 'tr-TR' },
...
function loadSettings() {
    try {
        const savedHistory = localStorage.getItem('echobridge_history');
        historyData = savedHistory ? JSON.parse(savedHistory) : [];
    } catch(e) { historyData = []; }

    try {
        const savedSpeed = localStorage.getItem('echobridge_speed');
        if (savedSpeed) currentVoiceSpeed = parseFloat(savedSpeed);
    } catch(e) {}

    try {
        const savedSource = localStorage.getItem('echobridge_source');
        const savedTarget = localStorage.getItem('echobridge_target');
        if (savedSource && LANG_DATA[savedSource]) currentSource = savedSource;
        if (savedTarget && LANG_DATA[savedTarget]) currentTarget = savedTarget;
    } catch(e) {}
}

function saveSettings() {
    try {
        localStorage.setItem('echobridge_speed', currentVoiceSpeed.toString());
        localStorage.setItem('echobridge_source', currentSource);
        localStorage.setItem('echobridge_target', currentTarget);
    } catch(e) {}
}
...
function speak(text, langCode) {
    return new Promise((resolve, reject) => {
        ...
        u.onerror = (e) => { isSpeaking = false; reject(e.error || 'Hata'); };
...
function updateUI() {
    ...
    // Ses hızı butonlarını güncelle
    document.querySelectorAll('.speed-opt').forEach(btn => {
        btn.classList.toggle('active', parseFloat(btn.dataset.speed) === currentVoiceSpeed);
    });
}
...
if (recognition) {
    ...
    recognition.onerror = (e) => {
        if (e.error !== 'aborted') console.warn('Ses tanıma hatası:', e.error);
        resetMicUI();
    };
...
function initPushToTalk(btnId, type) {
    ...
    const start = (e) => {
        ...
        if (btn.disabled || isPressed) return;
        isPressed = true;
        warmUpMic();
    ...
    // Hem pointer hem touch events (mobilde takılmayı önler)
    ...
    btn.addEventListener('touchend', (e) => { e.preventDefault(); stop(e); }, { passive: false });
}
...
function updateCharCount() {
    ...
    if (text.length >= 5000) counter.classList.add('critical');
    else if (text.length >= 4800) counter.classList.add('danger');
...
function autoTranslate() {
    ...
    // Cache kontrolü
    const cached = getCachedTranslation(trimmed, currentSource, currentTarget);
    if (cached) {
        document.getElementById('text-target').value = cached;
        lastSourceText = trimmed;
        lastTranslatedPair = `${trimmed}|${currentSource}|${currentTarget}`;
        return;
    }

    debounceTimer = setTimeout(() => { doTranslate(trimmed, currentSource, currentTarget); }, 700);
}
...
async function doTranslate(text, from, to, autoSpeak = false) {
    ...
    if (cached) {
        ...
        if (autoSpeak && !isSpeaking) {
            try { await speakText('target'); } catch (e) {}
        }
        return;
    }
    ...
    while (attempts > 0) {
        try {
            ...
            if (data && data[0]) {
                ...
                // Mikrofondan gelen metin çevrildikten sonra OTOMATİK seslendir
                if (autoSpeak && myId === currentTranslationId && !isSpeaking) {
                    try { await speakText('target'); } catch (e) {}
                }
...
async function speakText(which) {
    ...
    if (iconContainer) {
        ...
        try { 
            ...
            showToast(`Seslendirme hatası`, 'error'); 
        } finally { 
...
function shareTarget() {
    ...
    if (navigator.share) { 
        navigator.share({ title: 'EchoBridge Çeviri', text }).catch(() => {}); 
    } else { 
        fallbackCopy(document.getElementById('text-target'), () => {
...
function renderHistory() {
    ...
    list.innerHTML = historyData.map(item => {
        const fromLang = LANG_DATA[item.from] || { flag: '❓', label: item.from };
        const toLang = LANG_DATA[item.to] || { flag: '❓', label: item.to };
...
function initPWAInstall() {
    ...
    const btnDismiss = document.getElementById('btn-dismiss-pwa');
    ...
    btnInstall?.addEventListener('click', async () => {
        ...
        if (outcome === 'accepted') showToast('Ana ekrana eklendi!', 'success');
    });

    btnDismiss?.addEventListener('click', () => {
        banner.classList.add('hidden');
        localStorage.setItem('echobridge_pwa_dismissed', Date.now().toString());
    });

    // 7 gün geçtiyse tekrar göster
    const dismissed = localStorage.getItem('echobridge_pwa_dismissed');
    if (dismissed && Date.now() - parseInt(dismissed) < 7 * 24 * 60 * 60 * 1000) {
        // beforeinstallprompt event'i gelene kadar bekle
    }
}
